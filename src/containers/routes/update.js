  ...
  const handleUpdate = (v) => {
      ...
      updateOneRoute({
        variables: {
          ...
          data: {
            ...
            Pricing: [getVOrR('cabin4'), getVOrR('cabin3'), getVOrR('cabin2'), getVOrR('cabin1'), getVOrR('halflux'), getVOrR('lux')],
          },
        },
      })
    }
    if (uploadFile) {
      upload({
        ...
        onCompleted: (data) => doUpdate('https://yaktors.ru/image/' + data.singleUpload),
        onError: (er) => {
          console.log(er)
        },
      })
